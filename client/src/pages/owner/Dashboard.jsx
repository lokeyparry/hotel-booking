              ...
              <div className="flex justify-between flex-wrap gap-2 sm:grid grid-cols-[2fr_2fr_1fr_1fr] lg:grid-cols-[0.5fr_2fr_2fr_1fr_1fr] px-6 py-3  text-gray-50 medium-14 border-b-1 border-slate-900/15" key={index}>
                ...
                <div className="flexStart gap-x-2 max-w-64">
                  <div className="overflow-hidden rounded-lg">
                    <img src={booking.property.images[0]} className='w-16 rounded-lg' alt="" />
                  </div>
                  <div className="line-clamp-2">{booking.property.title}</div>
                </div>
                <div className="">{new Date(booking.checkInDate).toLocaleDateString()} to {new Date(booking.checkOutDate).toLocaleDateString()}</div>
                <div className="">{currency}{booking.totalPrice}</div>
                <button className={`${booking.isPaid ? "bg-green-400/80 text-white":"bg-secondary/10 text-red-500"} w-22 py-0.5 rounded-full text-xs border border-green-500/30`}>
